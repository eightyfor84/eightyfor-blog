...
const plugins: Record<string, PluginMapping> = {
  search: {
    ...
    format: 'yaml',
    name: 'Search 检索',
    description: '搜索建议 / 全局搜索 / 全文索引 — Search suggestions, global search, full-text index',
    featureFlag: 'enabled',
    builtin: true,
    storage: 'site',
  ...
  friends: {
    ...
    format: 'yaml',
    name: 'Friends 友链',
    description: '好友卡片与全局样式 — Friend cards & global style',
    featureFlag: 'enabled',
    builtin: true,
    storage: 'file',
    contentEditor: true,
  ...
  comments: {
    ...
    format: 'yaml',
    name: 'Comments 评论',
    description: '主开关与评论配置（后端/态度/图床）— Master switch, backend, attitude, image upload',
    featureFlag: 'enabled',
    builtin: true,
    storage: 'site',
  ...
  slides: {
    ...
    format: 'yaml',
    name: 'Slides 幻灯片',
    description: 'Marp 幻灯片渲染（post 级配置走 frontmatter slideshow 段）— Marp slide rendering',
    featureFlag: 'enabled',
    storage: 'post',
    builtin: true,
  },
  collections: {
    ...
    format: 'yaml',
    name: 'Collections 合集',
    description: '合集树与合集导航设置 — Collection tree & navigation settings',
    featureFlag: 'enabled',
    builtin: true,
    storage: 'file',
    contentEditor: true,
  ...
  readingExperience: {
    ...
    description: '目录与文末区块（相关/上下篇/分享/态度/作者卡）— TOC & end-of-article blocks',
    featureFlag: 'enabled',
    builtin: true,
    storage: 'site',
    entries: [{ target: 'chronicle:post-page' }],
